...
```mermaid
flowchart TB
    subgraph Vercel["Frontend (Bonus - Vercel)"]
        FE["React + Vite + Apollo Client + Leaflet"]
    end

    subgraph Railway1["Backend API Server (Railway)"]
        BE["Fastify + GraphQL Yoga + Pothos + Prisma"]
    end

    subgraph Railway2["Database (Railway)"]
        DB[("PostgreSQL 16")]
    end
